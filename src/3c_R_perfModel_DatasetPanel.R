...
resTest_boosting=readRDS("E:\\DataScientist\\project\\forecastingDEF\\results\\3_ModelSelection/df_resTest_DatasetPanel_boosting.RDS") 
prop.table(table(as.numeric(resTest_boosting$Y)))#0.01139515%
dim(resTest_boosting)

resTest_RF=readRDS("E:\\DataScientist\\project\\forecastingDEF\\results\\3_ModelSelection/df_resTest_DatasetPanel_RF.RDS") 
...
perfmeas_df <- rbind(perfmeas_boostingADA, perfmeas_boostingLOGIT,
                     ...
                     perfmeas_RF100, perfmeas_RF100_100l, perfmeas_RF50_50l, perfmeas_RF50) %>% 
...
saveRDS(perfmeas_df,"E:\\DataScientist\\project\\forecastingDEF\\results\\3_ModelSelection/perfmeas_df_DatasetPanel.RDS")


## Represent densities of best one:
# p <- resTest_boosting %>%
#   ggplot( aes(x=adaboost, fill=as.factor(Y))) +
#   geom_histogram( color="#e9ecef", alpha=0.6, position = 'identity') +
#   scale_fill_manual(values=c("#69b3a2", "#404080")) +
#   labs(fill="")
# 
# p2 <- ggplot(data=resTest_boosting, aes(x=adaboost, group=as.factor(Y), fill=as.factor(Y))) +
...
##gain curve
# p= resTest_boosting %>% 
#   mutate(Y_fact = as.factor(Y)) %>% 
#   gain_curve(Y_fact, xgboost) %>% 
#   autoplot()
# 
# ggplotly(p)
