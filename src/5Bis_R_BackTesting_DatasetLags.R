...
table(don_all$Y)
prop.table(table(as.numeric(don_all$Y)))#%0.009371442 
...
for(year_pred in 2018:2022){
  ...
  #adaboost
  #gbm_ada <- gbm(Y~., data=list_don_train[[as.character(year_pred)]], distribution="adaboost")
  #list_results[[as.character(year_pred)]]$adaboost  <- predict(gbm_ada, newdata=list_don_test[[as.character(year_pred)]], type="response")
  
  
  #xgboost  
  train_x = data.matrix(select(list_don_train[[as.character(year_pred)]],-Y))
  train_y = list_don_train[[as.character(year_pred)]]$Y
  test_x = data.matrix(select(list_don_test[[as.character(year_pred)]],-Y))
  test_y = list_don_test[[as.character(year_pred)]]$Y
  ...
  list_results[[as.character(year_pred)]]$xgboost <- predict(xgboost,xgb_test, type="prob")
  
  summary(list_results[[as.character(year_pred)]]$xgboost)
  list_results[[as.character(year_pred)]]$xgboost[list_results[[as.character(year_pred)]]$xgboost <0] <- 0
  list_results[[as.character(year_pred)]]$xgboost[list_results[[as.character(year_pred)]]$xgboost >1] <- 1
  
}


saveRDS(list_results,"E:\\DataScientist\\project\\forecastingDEF\\results\\4_BackTesting/df_DatasetLags_resTest_backtesting.RDS")
...
#list_results=readRDS("E:\\DataScientist\\project\\forecastingDEF\\results\\4_BackTesting/df_DatasetLags_resTest_backtesting.RDS") 

#compute AUC in different years:
auc=c()

for(iii in 2018:2022){
  auc[as.character(iii)] = as.numeric(auc(list_results[[as.character(iii)]]$Y,list_results[[as.character(iii)]]$xgboost))
}
...
roc2018 <- roc(list_results[["2018"]]$Y,list_results[["2018"]]$xgboost)
roc2019 <- roc(list_results[["2019"]]$Y,list_results[["2019"]]$xgboost)
roc2020 <- roc(list_results[["2020"]]$Y,list_results[["2020"]]$xgboost)
roc2021 <- roc(list_results[["2021"]]$Y,list_results[["2021"]]$xgboost)
roc2022 <- roc(list_results[["2022"]]$Y,list_results[["2022"]]$xgboost)
...
saveRDS(perfmeas_df,"E:\\DataScientist\\project\\forecastingDEF\\results\\4_BackTesting/perfmeas_df_DatasetLags_BackTesting.RDS")
write.csv(perfmeas_df,"E:\\DataScientist\\project\\forecastingDEF\\results\\4_BackTesting/perfmeas_df_DatasetLags_BackTesting.csv")
